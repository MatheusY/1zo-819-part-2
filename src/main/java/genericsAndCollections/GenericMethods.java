...
public class GenericMethods {
	...
	public static void main(String[] args) {
		...
		System.out.println(
				"compareObjects(Apple, Apple): Match = " + (g.compareObjects(new Apple(1), new Apple(1)) == 0));

		System.out.println(
				"compareComparables(Apple, Apple): Match = " + (g.compareComparables(new Apple(1), new Apple(1)) == 0));

		System.out.println("compareMatchingTypes(Apple, Apple): Match = "
				+ (g.<Apple>compareMatchingTypes(new Apple(1), new Apple(1)) == 0));

		// Compare Apple and Green Apple
		System.out.println("\n-------- Compare Apple to GreenApple -----");
		System.out.println("compareObjects(Apple, GreenApple): Match = "
				+ (g.compareObjects(new Apple(1), new GreenApple(1)) == 0));

		System.out.println("compareComparables(Apple, GreenApple): Match = "
				+ (g.compareComparables(new Apple(1), new GreenApple(1)) == 0));

		System.out.println("compareMatchingTypes(Apple, GreenApple): Match = "
				+ (g.<Apple>compareMatchingTypes(new Apple(1), new GreenApple(1)) == 0));

		// You know at compile time that you cannot compare
		// Apples and Oranges, but still you try....
		System.out.println("\n-------- Compare Apple to Orange -----");
//		System.out.println(
//				"compareObjects(Orange, Apple): Match = " + (g.compareObjects(new Orange(1), new Apple(1)) == 0));

//		System.out.println("compareMatchingTypes(Orange, Apple): Match = "
//				+ (g.<Apple>compareMatchingTypes(new Orange(1), new Apple(1)) == 0));
