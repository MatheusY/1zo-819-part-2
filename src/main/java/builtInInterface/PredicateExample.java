...
public class PredicateExample {
	public static void main(String[] args) {
		...
		// always returns true.
		Predicate b1 = s -> true;
		System.out.println("The value of " + "Predicate [s -> true].test(null) = " + b1.test(null));

		// Predicate with Object argument, lambda expression
		// returns true if arugment is null
		Predicate b2 = s -> s == null;
		System.out.println("The value of " + "Predicate [s -> s == null].test(null) = " + b2.test(null));

		// Predicate typed to Boolean, lambda expression
		// returns boolean value of the value passed
		Predicate<Boolean> b3 = b -> b;
		System.out.println("The value of Predicate [b -> b].test(" + "Boolean.valueOf('true')) = "
		...
		System.out.println("The value of Predicate [(String s) ->"
				+ " { return s.equals(\"hello\");}].test(\"hello\") = " + b4.test("hello"));
		...
		System.out.println("The value of Predicate negated [(String s) ->"
				+ " { return s.equals(\"hello\");}].test(\"hello\") = " + b5.test("hello"));
		...
		System.out.println("Executing firstTest[i > -10].or(secondTest" + "[i < 10]).test(-11)");
		System.out.println("\tResult = " + bFinal.test(-11));

		// Execute the joined predicates directly as shown:
		System.out.println("Executing firstTest[i > -10].and(secondTest" + "[i < 10]).test(0)");
		System.out.println("\tResult = " + firstTest.and(secondTest).test(0));

		// Short-circuiting and
		System.out.println("Executing firstTest[i > -10].and(secondTest" + "[i < 10]).test(-11)");
		System.out.println("\tResult = " + firstTest.and(secondTest).test(-11));

		// Short-circuiting or
		System.out.println("Executing firstTest[i > -10].or(secondTest" + "[i < 10]).test(0)");
		System.out.println("\tResult = " + firstTest.or(secondTest).test(0));

		// Add negate...
		System.out.println("Executing firstTest[i > -10].or(secondTest" + "[i < 10]).negate().test(0)");
		System.out.println("\tResult = " + firstTest.or(secondTest).negate().test(0));

		System.out.println("The value of Predicate.isEqual(\"hello\") with " + "value of \"hello\" = "
				+ Predicate.isEqual("hello").test("hello"));

		System.out.println("The value of Predicate.not(" + "Predicate.isEqual(\"hello\")) with value of \"hello\" = "
