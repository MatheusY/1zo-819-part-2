...
public class FunctionExample {
	public static void main(String[] args) {
		...
		System.out.println("Function [(s) -> s]. apply(\"hello\") = " + f1.apply("hello"));

		// Function which splits a string on space, returns 1st element
		Function<String, String> f2 = (s) -> s.split("\\s")[0];

		System.out.println(
				"Function [(s) -> s.split(\"\\\\s\")[0] ]." + " apply(\"hello there\") = " + f2.apply("hello there"));

		// Function returns sublist string with spaces not commas
		Function<List<Integer>, String> f3 = (s) -> s.subList(1, 4).toString().replace(',', ' ');

		System.out.println("Function" + " [s.subList(1,4).toString().replace(',',' ' ]."
				+ " apply(List.of(10,20,30,40,50)) = " + f3.apply(List.of(10, 20, 30, 40, 50)));

		// Parsing the parts of the lambda expression into pre and post

		// First, sublist
		Function<List<Integer>, List<Integer>> fPre = (s) -> {
			System.out.println("fPre executed");
			return s.subList(1, 4);
		};

		// Second, make it a String - main function
		Function<List<Integer>, String> fResult = (s) -> {
			System.out.println("fResult executed");
			return s.toString();
		};

		// After its a String, replace comma with space
		Function<String, String> fPost = (s) -> {
		...
		System.out.println("fResult.compose(fPre).andThen(fPost).apply = "
				+ fResult.compose(fPre).andThen(fPost).apply(List.of(10, 20, 30, 40, 50)));

		// Executing by chaining Function local variables...
		System.out.println("fResult.compose(fPre).andThen(fPost).apply = "
				+ fResult.andThen(fPost).compose(fPre).apply(List.of(10, 20, 30, 40, 50)));
